...
function prevPageMyProperty() {
    if (current_page > 1) {
        current_page--;
        getMyPropwery(current_page);
    }
}

function nextPageMyProperty() {
...
function getMyPropwery(page = 1) {
    searchMyProperty(page)
}
...
function createLink(page = 1, totalRows = 1, perPage = 1) {
    ...
    for (var i = page - 3; i <= page; i++) {
        if (i >= 1 && i != page) {
            // linkCollection += `<a href="javascript:getMyPropwery(${i})" id="btn_prev">${i}</a>`
            linkCollection += `<li><a href="javascript:getMyPropwery(${i})"">${i}</a></li>`
        }
    }

    //Push target page link for the current page
    linkCollection += `<li class="active"><a href="javascript:getMyPropwery(${page})"">${page}</a></li>`
    ...
    if (page != numPagesCollection) {
        linkCollection += `<li><a href="javascript:nextPageMyProperty()" id="btn_next" class="Next"> Next <i class="fa fa-chevron-right"></i></a></li>`
    } else {
