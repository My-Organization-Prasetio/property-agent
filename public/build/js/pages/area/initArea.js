...
export function viewAllArea() {
	$.ajax({
		url: rootApp + "api/area",
		type: "GET",
		dataType: "json",
		success: function (res) {
			...
			if ($(".table-area").length > 0) {
				$(".table-area").dataTable().fnDestroy();
				if ($("#table-body-area").length > 0) {
					$("#table-body-area").html(tbArea);
				}
				_initTableOwner("table-area");
			}
		},
