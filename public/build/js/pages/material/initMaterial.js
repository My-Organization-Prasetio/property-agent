var _initTableMaterial = function (className) {
	// Initialize
	$("." + className).DataTable({
		autoWidth: false,
		columnDefs: [
...
export function viewAllMaterial() {
	$.ajax({
		url: rootApp + "api/material",
		type: "GET",
		dataType: "json",
		success: function (res) {
			...
			$.each(res.data, function (index, key) {
				tbMaterialData += `<tr>
                    ...
                    <td>
                        Rp_ ${numberWithCommas(key.product_price)}
                        <span class="d-block font-size-sm text-muted">Margin Price: Rp_ ${numberWithCommas(
													key.product_margin_price
												)}</span>
                    </td>
                    <td>
                        Rp_ ${numberWithCommas(key.product_after_margin_price)}
                    </td>
                    <td>
			...
			// return tbMaterialData;
			if ($(".table-material").length > 0) {
				$(".table-material").dataTable().fnDestroy();
				if ($("#table-body-material").length > 0) {
					$("#table-body-material").html(tbMaterialData);
				}
				_initTableMaterial("table-material");
			}
		},
